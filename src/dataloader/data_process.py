...
from dataset.session import sliding_window, session_window, session_window_bgl, fixed_window
import shutil
import pickle
...
def process_dataset(data_dir, output_dir, log_file, dataset_name, window_type, window_size, step_size, train_size,
    ...
    if window_type == "sliding":
        ...
        if dataset_name == "hdfs":
            ...
            session_ids = np.array(list(window_df.keys()))
            session_idx = list(range(len(window_df)))
            print("Using random partition.")
            np.random.shuffle(session_idx)

            session_idx_train = session_idx[0:n_train]
            session_idx_test = session_idx[n_train:]
            session_id_train = session_ids[session_idx_train]
            session_id_test = session_ids[session_idx_test]

            train_window = {k: window_df[k] for k in session_id_train}
            test_window = {k: window_df[k] for k in session_id_test}
        else:
    ...
    with open(os.path.join(output_dir, f"{dataset_name}_{window_size}_train.pkl"), mode="wb") as f:
        pickle.dump(train_window, f)
    with open(os.path.join(output_dir, f"{dataset_name}_{window_size}_test.pkl"), mode="wb") as f:
