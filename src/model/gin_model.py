...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, dropout_rate, gnn_type='gin'):
        super(EncoderLayer, self).__init__()
        self.gcn_type = gnn_type
        self.self_conv_norm = nn.LayerNorm(hidden_size)
        if gnn_type == 'gin':
            self.self_conv = GINConv(nn=nn.Sequential(nn.Linear(in_features=hidden_size, out_features=ffn_size)))
        elif gnn_type == 'gine':
            self.self_conv = GINEConv(nn=nn.Sequential(nn.Linear(in_features=hidden_size, out_features=ffn_size)), edge_dim=1)
        else:
            raise RuntimeError(f"The specified model is not supported") 

        # self.self_conv_dropout = nn.Dropout(dropout_rate)

    def forward(self, feats, edge_index, weight):
        y = self.self_conv_norm(feats)
        if self.gcn_type == 'gin':
            y = self.self_conv(y, edge_index)
        else:
            y = self.self_conv(y, edge_index, weight)
        y = F.relu(y)
        return y
