...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, ffn_size, nhead, dropout_rate):
        super(EncoderLayer, self).__init__()

        self.self_conv_norm = nn.LayerNorm(hidden_size)
        self.self_conv = TransformerConv(in_channels=hidden_size,
                                         out_channels=ffn_size,
                                         heads=nhead,
                                         concat=False,
                                         beta=True,  # if True, for bgl 20 it is worse.
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        y = self.self_conv(y, edge_index, edge_weight)
        y = self.self_conv_dropout(y)
        x = x + y
